...
def add_new_articles(file, search_strategy):
    ...
    df1 = pd.DataFrame.from_records(data)[['pubmed_id']].sort_values(by=['pubmed_id']).reindex()
    ...
    df2 = pd.DataFrame.from_records(articles_data)[['pubmed_id']].sort_values(by=['pubmed_id']).reindex()
